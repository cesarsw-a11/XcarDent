document.addEventListener('DOMContentLoaded', () => {
    ...
    const itensProcedimento = [{
        ...
        nome: 'Obturación en buen estado',
        cor: '#FFFF00'
    }, {
        nome: 'Sellante de fosas y fisuras en buen estado',
        cor: '#FF0000'
    }, {
        nome: 'Diente indicado a extraccion',
        cor: '#000000'
    }, {
        nome: 'Corona en mal estado',
        cor: '#0000FF'
    }, {
        nome: 'Sellante de fosas y fisuras en mal estado',
        cor: '#FFC0CB'
    }, {
        nome: 'Fractura de corona dental',
        cor: '#8B0000'
    }, {
        nome: 'Caries dental',
        cor: '#FA8072'
        ...
        salvar() {
            ...
            if (this.valido()) {
                ...
                if (procedimento == undefined) {
                    $.ajax({
                        type: "POST",
                        data : {diente : this.numeroDente , caraDiente : this.faceDente, nombre:this.nome , color : this.cor ,paciente : $("#paciente").val(),
                    info : this.informacoesAdicionais},
                        url : base_url + "administrador/guardarDiente",
                    ...
                    alertify.alert("Este procedimiento ya existe en esta zona del diente.")
                    procedimentos[procedimentos.indexOf(procedimento)] = this.criaObjeto()
                }
        ...
        fetch() {
            $.ajax({
                ...
                success : function(data){
                    ...
                    localStorage.clear()
                    localStorage.setItem("procedimentos",data)
                }
            })
        ...
        procedimentos.filter(prc => prc.numeroDente == procedimento.numeroDente && prc.faceDente == procedimento.faceDente).forEach(item => {
            const tr = `
                <tr>
                    ...
                    <td>
                        ${item.informacoesAdicionais || 'Sin info adicional.'}
                    </td>
                    <td>
    ...
    window.apagar = (nome, numeroDente, faceDente) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: base_url + 'administrador/eliminarDiente',
                    type: 'POST',
                    data : {numeroDiente : numeroDente , caraDiente : faceDente, paciente :$("#paciente").val() },
    ...
    const resizeCanvas = () => {
        ...
        base_image.src = '../imagenes/dientes/18.png';
    ...
    const resizeCanvasPincel = () => {
        ...
        $.ajax({
            type:"POST",
            data : {paciente : $("#paciente").val()},
            url : base_url + "administrador/obtenerDiseno",
            success : function(data){
                data = JSON.parse(data)
                //console.log(data)
    ...
    const iniciaOdontograma = () => {
        ...
        document.querySelector("#botaoAdicionar").onclick = (event) => {
            ...
            if($("#nomeProcedimento").val() == "-1"){
                alertify.alert("Debe seleccionar un procedimiento")
                return false
            }
